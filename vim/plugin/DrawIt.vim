...
"
" Last Change: Aug 04, 2003	NOT RELEASED
...
"
" Variables:
"	g:drawit_insertmode : if this variable exists and is 1 then some
"             maps are made (the cursor control ones) are also available
"             in insert mode.
"
" History:
...
function! <SID>StartDrawIt()
  ...
  if exists("g:drawit_insertmode") && g:drawit_insertmode
   ...
   call <SID>SaveMap("i","","<s-left>")
   call <SID>SaveMap("i","","<s-right>")
   call <SID>SaveMap("i","","<s-up>")
   call <SID>SaveMap("i","","<s-down>")
   call <SID>SaveMap("i","","<home>")
   call <SID>SaveMap("i","","<end>")
   call <SID>SaveMap("i","","<pageup>")
   call <SID>SaveMap("i","","<pagedown>")
   call <SID>SaveMap("i","","<s-leftmouse>")
  endif
  let amap="\<c-v>"
  if maparg(amap,"n") != ""
  ...
  " start visual-block with s-LeftMouse
