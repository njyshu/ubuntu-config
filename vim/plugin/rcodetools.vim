...
function! <SID>RCT_completion(findstart, base)
	...
	for dline in data
		...
		let fri_data = system('qri -f plain ' . "'" .  selector . "'" . ' 2>/dev/null')
...
function! <SID>RCT_new_ri_window()
  ...
  execute 'nmap <buffer><silent> ,h :call' . s:sid . 'RCT_execute_ri(expand("<cWORD>"))<cr>'
endfunction

function! <SID>RCT_execute_ri(query_term)
  silent %delete _
  let term = matchstr(a:query_term, '\v[^,.;]+')
  let cmd = s:GetOption("RCT_ri_cmd", "qri -f plain ")
...
execute 'au Filetype ruby,eruby nmap <buffer><silent> ,h :exec "call ' .
         \ 'RCT_find_tag_or_ri(''" . expand("<cword>") . "'')"<cr>'
execute 'au Filetype ruby,eruby nmap <buffer><silent>' . s:GetOption("RCT_ri_binding", "<LocalLeader>r") .
        \ ' :call ' .  s:sid . 'RCT_smart_ri()<cr>'
execute 'au Filetype ruby,eruby nmap <buffer><silent>' . s:GetOption("RCT_toggle_binding", "<LocalLeader>t") .
